...
#define _IMX_SEL_INP_REGISTER(InpSel) \
          ((((InpSel) & 0xFF) * 4) + iomuxc_select_input_base)

#define _IMX_MAKE_INP_SEL(InpSelReg, InpSelVal) \
          (((((InpSelReg) - iomuxc_select_input_base) / 4) & 0xFF) | \
          (((InpSelVal) & 0x7) << 8))
 ...
 * less than the minimum value so that any valid Select Input value
 * is non-zero.
 */
...
/* ECSPI2 MUX initialization */
typedef enum {
	// CS0 (PAD_EIM_RW) GPIO2_IO26
	IMX_PAD_EIM_RW_GPIO2_IO26 = _IMX_MAKE_PADCFG(
		                    IMX_SRE_FAST,
		                    IMX_DSE_40_OHM,
		                    IMX_SPEED_MEDIUM,
		                    IMX_ODE_DISABLE,
		                    ...
		                    IMX_HYS_DISABLED,
		                    IMX_SION_DISABLED,
		                    IMX_IOMUXC_EIM_RW_ALT5_GPIO2_IO26),

	// CS1 (PAD_EIM_LBA) GPIO2_IO27
	IMX_PAD_EIM_LBA_ECSPI2_SS1 = _IMX_MAKE_PADCFG(
		                    IMX_SRE_FAST,
		                    IMX_DSE_40_OHM,
		                    IMX_SPEED_MEDIUM,
		                    IMX_ODE_DISABLE,
	...
	IMX_PAD_EIM_OE_ECSPI2_MISO = _IMX_MAKE_PADCFG(
		                    IMX_SRE_FAST,
		                    IMX_DSE_40_OHM,
		                    IMX_SPEED_MEDIUM,
		                    IMX_ODE_DISABLE,
	...
	IMX_PAD_EIM_CS1_ECSPI2_MOSI = _IMX_MAKE_PADCFG(
		                    IMX_SRE_FAST,
		                    IMX_DSE_40_OHM,
		                    IMX_SPEED_MEDIUM,
		                    IMX_ODE_DISABLE,
	...
	IMX_PAD_EIM_CS0_ECSPI2_SCLK = _IMX_MAKE_PADCFG(
		                    IMX_SRE_FAST,
		                    IMX_DSE_40_OHM,
		                    IMX_SPEED_MEDIUM,
		                    IMX_ODE_DISABLE,
...
static void imx_pad_config(IMX_PAD Pad, IMX_PADCFG PadConfig)
{
    ...
    write32(_IMX_PADCFG_MUX_CTL(PadConfig), iomuxc_base + _IMX_PAD_MUX_OFFSET(Pad));

    /* Configure Select Input Control */
    if (_IMX_PADCFG_SEL_INP(PadConfig) != 0) {
        write32(_IMX_SEL_INP_VALUE(_IMX_PADCFG_SEL_INP(PadConfig)),
...
bool imx_iomux_configure_spi(uint8_t spiBus, int32_t *bank, int32_t *pin)
{
    ...
    *pin = 26;
    ...
    misoSelectInputVA = (uint32_t)phys_to_virt(
        IOMUXC_ECSPI2_MISO_SELECT_INPUT,
        MEM_AREA_IO_SEC);
    initialValue = read32(misoSelectInputVA);
    newValue = (initialValue & (~3));
